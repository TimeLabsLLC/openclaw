...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.AGENTOS_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@agentos/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "agentos.plugin.json");
    ...
    if (packageJson.agentos && "extensions" in packageJson.agentos) {
      packageJson.agentos = {
        ...packageJson.agentos,
        extensions: rewritePackageExtensions(packageJson.agentos.extensions),
        ...(typeof packageJson.agentos.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.agentos.setupEntry) }
          : {}),
      };
    }
