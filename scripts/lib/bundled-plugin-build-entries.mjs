...
export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.agentos?.extensions)
    ? packageJson.agentos.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.agentos?.setupEntry === "string" &&
    packageJson.agentos.setupEntry.trim().length > 0
      ? packageJson.agentos.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "agentos.plugin.json"));
    }
    if (hasPackageJson) {
