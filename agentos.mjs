...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `agentos: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const respawnWithoutCompileCacheIfNeeded = () => {
  ...
  if (process.env.AGENTOS_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    AGENTOS_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
  };
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.AGENTOS_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    AGENTOS_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  const result = spawnSync(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:agentos/agentos#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
...
const isHelpFastPathDisabled = () => process.env.AGENTOS_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
